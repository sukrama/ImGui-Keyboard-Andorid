...
int32_t ImGui_ImplAndroid_HandleInputEvent(const AInputEvent* input_event)
{
    ...
    case AINPUT_EVENT_TYPE_MOTION:
    {
        ...
        int32_t tool_type_primary = AMotionEvent_getToolType(input_event, event_pointer_index);

        switch (tool_type_primary)
        {
        ...
        {
            int32_t pointer_count = AMotionEvent_getPointerCount(input_event);
            int tool_type = AMotionEvent_getToolType(input_event, event_pointer_index);
            bool is_down = (event_action == AMOTION_EVENT_ACTION_POINTER_DOWN);

            if (tool_type == AMOTION_EVENT_TOOL_TYPE_FINGER || tool_type == AMOTION_EVENT_TOOL_TYPE_UNKNOWN)
            {
                float x = AMotionEvent_getX(input_event, event_pointer_index);
                float y = AMotionEvent_getY(input_event, event_pointer_index);
                ...
                else if (event_pointer_index == 1)
                {
                    io.AddMouseButtonEvent(1, is_down);

                    if (pointer_count >= 2 && is_down)
                    {
                        float x0 = AMotionEvent_getX(input_event, 0);
...
void ImGui_ImplAndroid_NewFrame(int window_width, int window_height)
{
    ImGuiIO& io = ImGui::GetIO();
