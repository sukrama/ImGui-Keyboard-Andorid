    ...
    if (fp != NULL) {
        while (fgets(line, sizeof(line), fp)) {
            if (strstr(line, libraryName)) {
                ...
                // Parse addresses/offset into fixed-width temporaries: void* is 4
                // bytes on 32-bit ABIs, so writing %llx directly into them corrupts
                // adjacent struct fields.
                unsigned long long tmpStart = 0, tmpEnd = 0, tmpOffset = 0;
                sscanf(line, "%llx-%llx %s %llx %s %d %s",
                       &tmpStart, &tmpEnd,
                       tmpPerms, &tmpOffset, tmpDev, &retMap.inode, tmpPathname);

                retMap.startAddr = reinterpret_cast<void *>((uintptr_t) tmpStart);
                retMap.endAddr   = reinterpret_cast<void *>((uintptr_t) tmpEnd);
                retMap.offset    = (long) tmpOffset;
